...
PhotoSync — Android-приложение-компаньон для камеры, созданное для фотографов и энтузиастов обработки изображений. Подключите камеру напрямую через USB PTP — без Wi-Fi, без настольного ПО — авто-передача при съёмке, просмотр, масштабирование, экспорт и удаление фотографий RAW/JPEG прямо на телефоне.

## Функции

- **Прямое USB PTP-подключение** — подключение камеры через USB PTP, plug-and-play, без сети
- **Авто-передача при съёмке** — фотографии автоматически передаются на устройство по мере съёмки
- **Декодирование RAW** — LibRaw + lcms2 управление цветом, поддержка 20+ форматов: ARW, CR3, NEF, DNG, RAF и др.
- **Плавный просмотр** — загрузка миниатюр в первую очередь, плиточная отрисовка JPEG, масштабирование двумя пальцами
- **Экспорт без потерь** — прямое копирование JPEG в галерею, RAW декодируется и сохраняется, без сжатия, без облака
- **Управление фото** — удаление фотографий из локального хранилища
...
2. На камере выберите режим "PC Connection" или "PTP/MTP"
...
| База данных | Room |
| Обработка изображений | LibRaw 0.21.4 + lcms2 2.18 |
