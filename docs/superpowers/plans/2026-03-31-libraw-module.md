...
> **状态：已完成。** 下列步骤已全部执行并验证，复选框仅作为执行顺序的记录保留，
> 不再表示待办工作。最终目录结构见 `AGENTS.md`（第 2 节）与 `README.md`（项目结构）。
>
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement future plans task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
- [x] **Step 1: 在 `gradle/libs.versions.toml` 的 `[plugins]` 部分添加 android-library 插件**
...
- [x] **Step 2: 在根 `build.gradle` 中声明 android-library 插件（apply false）**
...
- [x] **Step 1: 在 `settings.gradle` 中注册 `:libraw` 模块**
...
- [x] **Step 2: 创建 `libraw/build.gradle`**
...
- [x] **Step 3: 创建 `libraw/src/main/AndroidManifest.xml`**
...
- [x] **Step 1: 添加 LibRaw submodule，固定到 tag 0.21.4**
...
- [x] **Step 2: 添加 Little-CMS submodule，固定到最新稳定 tag**
...
- [x] **Step 3: 验证 submodule 目录中存在关键文件**
...
- [x] **Step 1: 创建 `libraw/src/main/cpp/CMakeLists.txt`**
...
- [x] **Step 2: 验证 CMakeLists.txt 语法（不执行编译）**
...
- [x] **Step 1: 复制 `libraw_jni.cpp` 到新模块**
...
- [x] **Step 2: 修改 `libraw_jni.cpp` 的 include 路径**
...
- [x] **Step 3: 复制 `RawWrapper.kt` 到新模块并添加 `System.loadLibrary`**
...
- [x] **Step 1: 移除 `defaultConfig` 中的 `externalNativeBuild` 和 `ndk` 块**
...
- [x] **Step 2: 移除顶级 `externalNativeBuild` 块**
...
- [x] **Step 3: 在 `dependencies` 块中添加 `:libraw` 依赖**
...
- [x] **Step 4: 验证 `app/build.gradle` 中不再有 native 相关配置**
...
- [x] **Step 2: 删除旧的 `RawWrapper.kt`**

```bash
rm app/src/main/java/cn/devcxl/photosync/wrapper/RawWrapper.kt
```

- [x] **Step 3: 确认 app 模块中不再残留 native/wrapper 相关文件**
